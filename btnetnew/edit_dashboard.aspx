...
function add_selected_report(chart_type, id)
{
    ...
    newform.name = 'addform';
    newform.id = 'addform';
    newform.method = 'GET';
    newform.action = 'update_dashboard.aspx';

    for (var i = 0; i < frm.elements.length; i++) {
        var el = frm.elements[i];

        if (el.attributes.getNamedItem("type").value == "hidden" && el.name.indexOf("__") != 0) {
            newform.appendChild(el.cloneNode(false));
        }
    }
