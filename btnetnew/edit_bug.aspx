...
<script lang="C#" runat="server">
    ...
    void Page_Load(Object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                dr_bug = Bug.get_bug_datarow(id, security, ds_custom_cols);
    ...
    void prepare_for_insert()
    {
        ...
        {
            Util.display_bug_not_found(Response, security, id); // TODO wrong message
            return;
        }

        titl.InnerText = Util.get_setting("AppTitle", "BugTracker.NET") + " - Create ";
        titl.InnerText += Util.capitalize_first_letter(Util.get_setting("SingularBugLabel", "bug"));
    ...
    void prepare_for_update()
    {
        ...
        if (dr_bug == null)
        {
            Util.display_bug_not_found(Response, security, id);
            return;
        }
        ...
        {
            Util.display_you_dont_have_permission(Response, security);
            return;
        }
        ...
        foreach (DataRow drcc in ds_custom_cols.Tables[0].Rows)
        {
            ...
            if (security.user.dict_custom_field_permission_level[column_name] != Security.PERMISSION_NONE)
            {
                string val = Util.format_db_value(dr_bug[column_name]);
    ...
    void do_insert()
    {
        ...
        WhatsNew.add_news(id, short_desc.Value, "added", security);
...
");
        ...
        WhatsNew.add_news(id, short_desc.Value, "updated", security);
        ...
        {
            Bug.send_notifications(Bug.UPDATE, id, security, 0,
                status_changed,
                assigned_to_changed,
                Convert.ToInt32(assigned_to.SelectedItem.Value));
        ...
        if (bug_fields_have_changed)
        {
            ...
            DataRow updated_bug = Bug.get_bug_datarow(id, security, ds_custom_cols);
            ...
            if (before.Trim() != after.Trim())
            {
                ...
                if ((string)drcc["dropdown type"] == "users")
                {
                    ...
                    else
                    {
                        ...
                        before = (string)DbUtil.execute_scalar(sql_get_username.Replace("$1", Util.sanitize_integer(before)));
                    }
                    ...
                    {
                        sql_get_username = "select us_username from users where us_id = $1";
                        after = (string)DbUtil.execute_scalar(sql_get_username.Replace("$1", Util.sanitize_integer(after)));
                    }
                }
        ...
        foreach (DataRow drcc in ds_custom_cols.Tables[0].Rows)
        {
            ...
            if (val.Length > 0)
            {
                ...
                else if (datatype == "decimal")
                {
                    ...
                    string decimal_error = Util.is_valid_decimal(name, val, xprec - xscale, xscale);
                    if (decimal_error != "")
                    {
